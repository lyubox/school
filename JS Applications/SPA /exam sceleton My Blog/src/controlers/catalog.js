...
export async function homePage () {
  ...
  if (this.app.userData === null) this.partial('./templates/catalog/homePage.hbs');
  else {
    const _posts = await getAll();
    const posts = _posts.map(p => ({ ...p, own: p._ownerId === this.app.userData.localId }));
    console.log({ posts });
    ...
    this.partials.singleItem = await this.load('/templates/catalog/singleItem.hbs');
    this.partials.createPost = await this.load('/templates/catalog/createPost.hbs');
